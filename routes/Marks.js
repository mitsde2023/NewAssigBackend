            ...
            if (foundUser) {
                ...
                if (userMarksData.length > 0) {
                    ...
                    if (updatedRowsCount > 0) {
                        console.log(`Subject Marks Data updated successfully for user_id ${userId} and subject_id ${class_id}.`);
                    } else {
                        await FlattenedDataModel.create(userData);
                        console.log(`No matching records found. New record inserted for user_id ${userId} and subject_id ${class_id}.`);
                    }
                } else {
                    console.log(`No data available for user_id ${userId} and subject_id ${class_id}.`);
                }
            }
        }
...
router.post('/subject_marks_update/:subject_id/:subject_name', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Subject Marks Data saved successfully.' });
    } catch (error) {
        console.error('Error updating subject marks data:', error);
        res.status(500).json({ success: false, message: 'Error saving subject marks data.' });
    }
});
        ...
        for (const item of data) {
            ...
            try {
                ...
                for (const userId of uniqueUserIds) {
                    ...
                    // if (foundUser) {
                    //     userData.name = foundUser[1];
                    //     userData.userUsername = foundUser[2];
                    // }
                ...
                for (const userData of flattenedData) {
                    ...
                    try {
                        ...
                        if (existingRecord) {
                            // Check if the record was updated more than 12 hours ago
                            const updatedAtThreshold = new Date();
                            updatedAtThreshold.setHours(updatedAtThreshold.getHours() - 25);
                            if (existingRecord.updatedAt < updatedAtThreshold) {
                                ...
                                console.log(`Subject Marks Data for user_id ${user_id} and subject_id ${subject_id} was updated in the last 12 hours. Skipping update.`);
                            }
                        } else {
                            // Create new record
                            await FlattenedDataModel.create(userData);
                            console.log(`No matching records found. New record inserted for user_id ${user_id} and subject_id ${subject_id}.`);
                        }
                    } catch (updateError) {
                        console.error(`Error updating/inserting subject marks data for user_id ${user_id} and subject_id ${subject_id}:`, updateError);
                    }
                }
...
// first Time Bulk create All Date ----------------------------------------------------------------------
    ...
    try {
        ...
        for (const item of data) {
            ...
            try {
                ...
                // Iterate over the flattenedData array
                // for (const userData of flattenedData) {
                //     const { user_id, subject_id ,subject_name} = userData;
                //     try {
                //         const [record, created] = await FlattenedDataModel.findOrCreate({
                //             where: { user_id, subject_id },
                //             defaults: { assignments: userData.assignments, subject_name },
                //         });

                //         if (!created) {
                //             // Record already existed
                //             console.log(`Subject Marks Data for user_id ${user_id} and subject_id ${subject_id} already exists. Skipping.`);
                //         } else {
                //             console.log(`New record inserted for user_id ${user_id} and subject_id ${subject_id}.`);
                //         }
                //     } catch (error) {
                //         console.error(`Error finding/creating subject marks data for user_id ${user_id} and subject_id ${subject_id}:`, error);
                //     }
                // }
                // await Log.create({
                //     message: `Subject Marks Data processed successfully for class_id: ${class_id}`,
                //     level: 'info',
                // });
            } catch (error) {
...
router.post('/student_subject_marks_data', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Subject Marks Data saved successfully.', data: subjectArray });
    } catch (error) {
